...
- **Focused-track handle**: `focusedTrack` — a computed `ProjectTrack | null` (null on the 4-track overview). Panels receive their reactive engine slice from this (`focusedTrack.engines.<engine>`) via a single `:params` prop and bind knobs directly to its fields (see §10 and D14). This replaced the ~30 per-field `trackParam` writable-computeds.
...
|---|---|
| Add a new engine type | `engine/KickEngine.ts` (smallest), `engine/types.ts`, `composables/useSynth.ts` (`engineFactories`, `ENGINE_SLICES`), plus a panel taking a `:params` prop and a `<template v-else-if="focusedTrack!.engineType === '…'">` branch in `App.vue` |
...
*Last updated: 2026-05-31 (auth & persistence foundation — optional Supabase Google sign-in, JWT-on-hello verified locally, app-owned `profiles` table via `ProfileStore`, guest path preserved; see §17 + D16). Prior: 2026-05-30 (variable track length — per-track `patternLength` 1–64 over a fixed 64-step buffer; sequencer emits an absolute counter, consumers apply per-track modulo; `schemaVersion` bumped to 2; see D15). Prior: 2026-05-30 (slice-down panel binding — panels bind the reactive engine slice via `:params`; `trackParam` wall + `engineType`/`synthMode` computeds removed; `focusedTrack` added; see D14). Prior: 2026-05-29 (sync protocol implemented + merged; Task 16b knob plumbing; server esbuild packaging; deployed Vercel + Render; presence-roster fix). When the contracts in §3 / §11 change, or the sync model in §15 evolves, update this doc — `CODE_REVIEW.md`, `ROADMAP.md`, and the memories (`audio_engine_decisions`, `sync_suppression_mechanism`, `project_state`) are the other places to keep in sync.*
