    ...
    <div class="rack-column">
      <div class="module-group clap-panel">
        ...
        <div class="knob-row">
          ...
          <Knob label="Tone" :min="500" :max="3000" :step="10" :defaultValue="DEFAULTS.tone" format="hz" v-model="tone" :syncPath="ks.pathFor('tone')" @gesture-end="ks.end('tone')" />
          <Knob label="Sloppy" :min="0.005" :max="0.03" :step="0.001" :defaultValue="DEFAULTS.sloppy" format="ms" v-model="sloppy" :syncPath="ks.pathFor('sloppy')" @gesture-end="ks.end('sloppy')" />
        </div>
      </div>
    </div>
...
const sloppy = defineModel<number>('sloppy', { required: true });
</script>
