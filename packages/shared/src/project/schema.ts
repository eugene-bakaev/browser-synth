...
// snare2: GENERATED from the descriptor table (single source of truth), same as
// kick2 — one z.number().min().max() leaf per row; ranges track snare2.ts.
const Snare2ParamsSchema = z.object(
  Object.fromEntries(
    SNARE2_DESCRIPTORS.map((d) => [d.key, z.number().min(d.min).max(d.max)]),
  ) as Record<string, z.ZodNumber>,
);

// hat2: GENERATED from the descriptor table (single source of truth), same as
// kick2/snare2 — one z.number().min().max() leaf per row; ranges track hat2.ts.
const Hat2ParamsSchema = z.object(
  Object.fromEntries(
    HAT2_DESCRIPTORS.map((d) => [d.key, z.number().min(d.min).max(d.max)]),
  ) as Record<string, z.ZodNumber>,
);
