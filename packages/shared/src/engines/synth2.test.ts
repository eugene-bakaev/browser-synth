...
describe('DEFAULT_SYNTH2_PARAMS', () => {
  ...
  it('defaults env2 to the same a/d/s/r as env1, loop off', () => {
    expect(DEFAULT_SYNTH2_PARAMS.env2).toEqual({ a: 0.01, d: 0.2, s: 0.5, r: 0.5, loop: false, sync: false, aDiv: '1/2', dDiv: '2', rDiv: '4' });
  });
  ...
  it('defaults env3 to a 0.2 / d 0.3 / s 0 / r 0.3, loop off (I3c)', () => {
    expect(DEFAULT_SYNTH2_PARAMS.env3).toEqual({ a: 0.2, d: 0.3, s: 0, r: 0.3, loop: false, sync: false, aDiv: '1/2', dDiv: '2', rDiv: '4' });
  });
  ...
  it('defaults each envelope tempo-sync off with 1/2 / 2 / 4 step divisions (step vocabulary, 2026-07-08)', () => {
    for (const env of [DEFAULT_SYNTH2_PARAMS.env1, DEFAULT_SYNTH2_PARAMS.env2, DEFAULT_SYNTH2_PARAMS.env3]) {
      ...
      expect(env.aDiv).toBe('1/2');
      expect(env.dDiv).toBe('2');
